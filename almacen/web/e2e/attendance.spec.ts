...
test('fichar en la caja sin internet (también quien no cobra) y el dueño lo ve en Horarios', async ({ page, context }) => {
  ...
  await page.getByRole('button', { name: es.pos.clockInOut }).click();
  ...
  await dialog.getByRole('button', { name: es.pos.clockIn }).click();
  ...
  await expect(page.getByRole('heading', { name: es.attendance.title })).toBeVisible();
  await expect(page.locator('.attendance-row', { hasText: 'Lucas' })).toContainText(es.attendance.working);
...
test('el dueño carga la ficha y el horario de un empleado', async ({ page }) => {
  ...
  await page.locator('.card', { hasText: '@sofia' }).getByRole('button', { name: es.team.ficha }).click();
  const dialog = page.getByRole('dialog');
  await dialog.getByLabel(es.team.dni).fill('40.111.222');
  await dialog.getByLabel(`${es.team.days.d1} ${es.attendance.in}`).fill('09:00');
  await dialog.getByLabel(`${es.team.days.d1} ${es.attendance.out}`).fill('17:00');
  await dialog.getByRole('button', { name: es.common.save }).click();
