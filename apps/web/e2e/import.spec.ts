...
test('importar una planilla y después imprimir sus etiquetas con precio por kilo', async ({ page }) => {
  ...
  await page.getByRole('button', { name: zh.importer.import.replace('{{count}}', '2') }).click();
  await expect(page.getByText(zh.importer.done.replace('{{created}}', '2').replace('{{updated}}', '0').replace('{{unchanged}}', '0'))).toBeVisible();
  ...
  await page.getByRole('button', { name: zh.labels.print }).click();
