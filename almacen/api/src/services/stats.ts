...
export async function avgDailySales(db: Db, storeId: string, productIds?: string[], window = 28) {
  ...
  const [rows, used, first] = await Promise.all([
    db.saleItem.groupBy({
      by: ['productId'],
      where: { sale: { storeId, status: 'COMPLETED', occurredAt: { gte: since } }, ...(productIds ? { productId: { in: productIds } } : {}) },
      _sum: { qty: true },
    }),
    // Lo que se usó de cada ingrediente en productos con receta (ej. el pan de los tostados).
    db.saleItemLot.groupBy({
      by: ['productId'],
      where: { productId: productIds ? { in: productIds } : { not: null }, saleItem: { sale: { storeId, status: 'COMPLETED', occurredAt: { gte: since } } } },
      _sum: { qty: true },
    }),
  ...
  const perDay = new Map(rows.map((r) => [r.productId, num(r._sum.qty) / span]));
  for (const u of used) if (u.productId) perDay.set(u.productId, (perDay.get(u.productId) ?? 0) + num(u._sum.qty) / span);
  return perDay;
}
