...
from memebot.agents.director import CloseSignal, Director, DirectorReport, Thesis
...
def metrics(mint: str, **o: Any) -> PairMetrics:
    ...
    base = dict(
        mint=mint, symbol=mint.upper(), price_usd=0.01, liquidity_usd=200_000, market_cap_usd=2_000_000,
        volume_m5=50_000, volume_h1=300_000, volume_h24=2_000_000, change_m5=5, change_h1=60, change_h6=80,
        change_h24=120, buys_m5=120, sells_m5=60, buys_h1=1200, sells_h1=600, created_at=NOW - 5 * 3600,
    )
...
class FakeLLM:
    ...
    director: DirectorReport = field(default_factory=lambda: DirectorReport(market_view="", theses=[], close_positions=[]))
    quant: QuantVerdict = field(
        default_factory=lambda: QuantVerdict(approved=True, confidence=0.7, reasoning="los datos acompañan", red_flags=[])
    )
    risk: RiskDecision = field(
        default_factory=lambda: RiskDecision(
            approve=True, size_pct=20, stop_loss_pct=20, take_profit_pct=50, trailing_stop_pct=25,
            max_hold_minutes=120, rationale="ok",
        )
    )
    over_budget: bool = False

    def ask(self, *, agent: str, schema: Any, **_: Any) -> Any:
        if self.over_budget:
...
def thesis(mint: str, conviction: float = 0.8) -> Thesis:
    return Thesis(
        mint=mint, symbol=mint.upper(), thesis="volumen acelerando y compras dominando",
        invalidation="las ventas pasan a dominar", conviction=conviction, horizon_minutes=90,
    )
...
def make_env(tmp_path):
    def build(cash: float = 1000, **overrides: Any) -> Env:
        cfg = cfg_with(**{"PAPER_FEE_USD": 0, "PAPER_SLIPPAGE_PCT": 0, **overrides})
        ...
        notes: list[str] = []
        bot = Bot(
            cfg=cfg, state=state, store=store, scanner=Scanner(market, safety, execution, cfg, now=now),
            agents=agents, notify=notes.append, now=now,
        )
        return Env(cfg, market, safety, llm, store, state, bot, notes, clock)
