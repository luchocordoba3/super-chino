...
export async function seedDemo() {
  ...
  const store = existing
    ? await prisma.store.update({ where: { id: existing.id }, data: { name: 'Supermercado Demo', settings: {} } })
    : await prisma.store.create({ data: { name: 'Supermercado Demo', code: DEMO_CODE } });
