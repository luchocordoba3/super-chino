...
test('dueño en el celular (chino): todas las pantallas sin errores', async ({ page }) => {
  ...
  const found = await visit(page, OWNER, zh.common.error, true);
  await page.goto('/settings');
  await expect(page.getByRole('button', { name: zh.settings.resetDemo })).toBeVisible();
  await page.goto('/products');
  await page.locator('tbody tr').first().click();
  await expect(page.getByRole('heading', { name: zh.products.priceHistory })).toBeVisible();
