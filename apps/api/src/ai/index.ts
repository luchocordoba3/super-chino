...
import { z } from 'zod';
import type { Lang } from '@super-chino/shared';
import { env } from '../env';
...
const PROMPTS = {
  invoice: `Esta es una foto de una factura o remito de un proveedor de un supermercado en Argentina. Extraé:
...
No inventes datos: lo que no se lea claramente va en null. Ignorá subtotales, impuestos, descuentos globales y totales.`,
  label: `Esta es una foto de la etiqueta o el envase de un producto de supermercado. Extraé:
...
export const aiService = {
  ...
  async translate(text: string, from: Lang, to: Lang): Promise<AiResult<string>> {
    const res = await getClient().beta.messages.create({
      ...
      system:
        `Traducís mensajes internos de un supermercado en Argentina entre el dueño y sus empleados, ` +
