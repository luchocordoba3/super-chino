...
- **Carta con QR en las mesas:** en *Ajustes → Carta con QR* se imprimen los carteles de las mesas, cada uno con el QR de la carta (se abre con la cámara, sin cuenta ni app) y, si Mercado Pago está conectado, el QR para pagar. En la carta aparecen los productos marcados *En la carta*, con lo que no hay marcado como "sin stock". Lo que pide el cliente llega a la cuenta de su mesa como **pendiente**: en la caja suena, aparece "Pidieron desde la mesa" y el vendedor acepta o rechaza (solo lo aceptado se cobra). El cliente ve su cuenta y puede tocar *Pedir la cuenta* (en la caja la mesa muestra 🧾). Cada link lleva un token secreto por mesa, hay límite de pedidos y se puede cambiar el QR de una mesa si hace falta.
- **Pedidos por WhatsApp:** en *Ajustes → Pedidos por WhatsApp* se crea el link del local (`/p/...`), con el número de WhatsApp, si hay envíos y un aviso (horarios, costo). Se comparte con un toque. El cliente ve el catálogo con precios y stock reales (lo que no hay aparece sin stock), arma el pedido (envío o retiro, cómo paga) y queda numerado. Después lo manda por WhatsApp con el texto ya escrito. El pedido llega a la caja (🛵 *Pedidos*, con sonido) y a la pantalla *Pedidos* del equipo: *Preparando*, *Listo* (abre WhatsApp con "tu pedido está listo"), *Cancelar* y, en la caja, *Cobrar*, que lo carga en la venta y al cobrarlo queda entregado. La demo lo tiene en `/p/demo`.
- **Factura electrónica ARCA (solo si el cliente la pide):** en *Ajustes → Factura electrónica (ARCA)* se cargan CUIT, razón social, condición (monotributo → factura C; responsable inscripto → B a consumidor final o A si el cliente da su CUIT), punto de venta, el certificado y la clave de ARCA (se guardan cifrados), el ambiente (homologación o producción) y *Probar conexión*. En la caja, después de cobrar (o desde *Últimas ventas*), *🧾 Factura*: consumidor final, DNI o CUIT. El pedido viaja en la cola de la caja (sin internet queda esperando) y el servidor pide el CAE a ARCA, integrado directo con WSAA (firma CMS) y WSFEv1, sin servicios pagos. Sale el link de la factura con el QR de ARCA para mandar por WhatsApp o imprimir. Si ARCA rechaza, queda el motivo y se reintenta desde la caja o desde *Ventas → Facturas*. Si la respuesta se cortó, antes de pedir otro número se consulta si ese ya quedó autorizado, así no se duplica. Las facturas A y B discriminan un solo IVA (el que se elige en Ajustes). Anular una venta facturada todavía no hace la nota de crédito: hay que hacerla aparte.
...
Además de las variables de Super Chino: `PUBLIC_URL`, `MP_CLIENT_ID`, `MP_CLIENT_SECRET`, `MP_WEBHOOK_SECRET` y `SECRETS_KEY` (ver `.env.example`). Las pruebas usan Mercado Pago y ARCA simulados (`web/e2e/fake-mp.mjs`, con un certificado autofirmado solo para pruebas en `web/e2e/fixtures`). Para probar con los de verdad, usá las credenciales de prueba de tu aplicación de Mercado Pago y el ambiente de homologación de ARCA.
